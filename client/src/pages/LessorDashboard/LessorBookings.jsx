...
import './LessorBookings.css';

const LessorBookings = ({ showSuccess, showError }) => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/lessors/all-bookings', {
        headers: {
      ...
      if (response.ok) {
        ...
        showError('Failed to fetch bookings', 'Please try refreshing the page');
      }
    } catch (error) {
      console.error('Error fetching bookings:', error);
      showError('Error loading bookings', 'Please check your connection and try again');
    } finally {
      setLoading(false);
    }
  };
